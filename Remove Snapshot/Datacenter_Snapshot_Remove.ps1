...
# === CONFIG ===
$logPath = "C:\tmp\vcenter_snapshot_cleanup_$(Get-Date -Format 'yyyy-MM-dd_HH-mm-ss').log"
$elasticHost = URI Address"
...
$MinFreeToSnapshotRatio = 2
# Force TLS 1.2+
[Net.ServicePointManager]::SecurityProtocol = [Nset.SecurityProtocolType]::Tls12
function Write-Log {
    param(
        [string]$message,
        [string]$level = "INFO"
    ...
    foreach ($snapshot in $snapshots) {
        ...
        if ($SnapshotDaysOld -gt $Max_Day) {
            ...
            if ($matchedElasticStorage.Type -ne "local") {
                if ($SnapshotSizeTB -le ($PhysicalFreeTB - $Max_Limit)) {
                    $canDelete = $true
                }
                elseif (
                    $PhysicalFreeTB -gt $MinSafeFreeTB -and 
            ($PhysicalFreeTB / $SnapshotSizeTB) -gt $MinFreeToSnapshotRatio
                ) {
                    Write-Log "INFO: Storage has <$Max_Limit TB free, but meets alternative deletion condition (>$MinSafeFreeTB TB free and ratio > $MinFreeToSnapshotRatio)." -level INFO
                    $canDelete = $true
                }
                else {
                    Write-Log "SKIPPED: Deleting snapshot '$($snapshot.Name)' would drop storage below limits. PhysicalFreeTB=$PhysicalFreeTB, SnapshotSizeTB=$SnapshotSizeTB" -level WARNING
                }
            }
            else {
                $canDelete = $true
