# === CONFIG ===
$logPath = "$env:USERPROFILE\Documents\vcenter_snapshot_cleanup_$(Get-Date -Format 'yyyy-MM-dd_HH-mm-ss').log"
$elasticHost = "URI ADRESS:9200"
...
# === CONNECT TO VCENTER ===
$VCenter = Connect-VIServer -Server #VC SERVER | Out-Null
Write-Log "Connected to vCenter."

# === COLLECT ALL CLUSTERS ===
$snapshotGroups = @()
$clusters = Get-Cluster | Where-Object { $_.name -ne  "sjc04-cluster" }
$snapshots = @()
foreach ($cluster in $clusters) {
    Write-Log "Processing cluster: $($cluster.Name)" -level "SECTION"

    $vms = Get-VM -Location $cluster
    Write-Log "Found $($vms.Count) VMs in cluster '$($cluster.Name)'"
        ...
        if ($SnapshotDaysOld -gt $Max_Day) {
            ...
            # === Delete Snapshot  === 
                ...
                try {
                    ...
                    $TotalDeletedSizeGB += $SnapshotSize
...
<table>
  ...
  <tr><td>Total Space Freed (TB)</td><td>$TotalDeletedSizeTB</td></tr>
